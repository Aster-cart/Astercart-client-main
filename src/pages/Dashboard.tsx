    ...
    <div>
      ...
      <div className="flex w-full font-inter justify-between py-2 flex-wrap gap-2">
        {[
          ...
          { label: "Platform Fee (10%)", value: (mockDashboardData as any).platformFee || "₦0", highlight: true },
        ].map((s, i) => (
          <div key={i} className="flex flex-col justify-between p-3 border bg-white border-border rounded-lg h-[70px] flex-1 min-w-[160px] mx-1">
            <span className="text-sm leading-4 text-body">{s.label}</span>
            <span className={`font-medium text-xl leading-7 ${s.highlight ? "text-pry" : "text-ink"}`}>
              {s.value}
            </span>
          </div>
        ))}
      ...
      {/* Transaction table */}
      <div className="mt-1 mx-2 p-3 font-inter bg-white rounded-2xl border border-border">
          ...
          <div className="flex gap-3 items-center flex-wrap">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="pl-10 pr-4 py-2 border rounded-lg text-xs border-border focus:outline-none focus:border-pry w-64 bg-off-white"
              />
            </div>
            ...
            <div className="relative">
              <button
                className="bg-off-white border border-border text-body rounded-lg px-3 py-2 flex items-center gap-2 text-sm hover:bg-pry-light transition-colors"
              ...
              {isOpen && (
                <div className="absolute right-0 bg-white border border-border rounded-lg mt-1 w-48 shadow-lg z-10">
                  {["All Transactions", "Recent Transaction"].map((opt) => (
                    <button
                      key={opt}
                      className="block w-full text-left px-4 py-2 text-sm text-body hover:bg-pry-light"
          ...
          <table className="w-full text-xs text-left">
            ...
            <tbody>
              {rows.length === 0 ? (
                ...
                rows.map((row, i) => (
                  <tr key={row.id} className="border-b border-border hover:bg-pry-light transition-colors">
                    <td className="px-2 py-3 text-muted">{String(i + 1).padStart(2, "0")}</td>
                    <td className="px-2 py-3 text-body">{row.user}</td>
                    <td className="px-2 py-3 font-mono text-body">{row.orderNo || row.transactionId}</td>
