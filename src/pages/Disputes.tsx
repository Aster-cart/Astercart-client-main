...
const Disputes: React.FC = () => {
  ...
  if (loading) return <p className="p-8 text-muted">Loading complaints...</p>;
    ...
    return (
      <div className="font-inter">
        ...
        <div className="bg-white rounded-xl p-6 border mb-4">
          ...
          {/* Customer contact */}
          <div className="grid grid-cols-2 gap-4 mb-4 bg-off-white rounded-lg p-3">
            {selected.email && <div><p className="text-xs text-muted">Email</p><p className="text-sm font-medium">{selected.email}</p></div>}
            {selected.phone && <div><p className="text-xs text-muted">Phone</p><p className="text-sm font-medium">{selected.phone}</p></div>}
          </div>
          ...
          {selected.status !== "resolved" && selected.status !== "rejected" && (
            <div>
              ...
              <textarea
                ...
                className="w-full border border-border rounded-lg p-3 text-sm focus:outline-none focus:border-pry resize-none"
              />
      ...
      <div className="bg-white rounded-xl border overflow-x-auto">
        {disputes.length === 0 ? (
          <div className="p-8 text-center">
            ...
            <p className="text-xs text-muted mt-1">Customer complaints about your store will appear here</p>
          </div>
        ) : (
          <table className="w-full text-sm text-left">
            ...
            <tbody>
              {disputes.map(d => (
                <tr key={d._id} className={`border-b hover:bg-off-white ${d.status === "open" ? "bg-red-50" : ""}`}>
                  <td className="py-3 px-4 font-medium">{d.customerName}</td>
                  <td className="px-4">{d.issueType}</td>
                  <td className="px-4 text-muted text-xs">{d.orderId ? `#${d.orderId}` : "—"}</td>
                  <td className="px-4">
